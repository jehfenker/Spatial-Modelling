...
buffer_dist         <- 3    # the buffer distance in map units (presumably decimal degrees)
...
output_model_extent        <- extent(-80, -33, -35, 8)    # the maximum extent for all lineage models
lineage_field_name  <- "lineage"   # the column for lineage name in the site data
...
named_species       <- c()          # a changeable list to allow for species in the dataset to be skipped
use_list            <- ""           # specify what to do with the named species:
...
SDM_model_base     <- paste(base_dir, "species_models/", sep="")
...
for (group in groups) {
  ...
  # skip if there are no points after cropping
  if (length(thisGroupPoints) == 0) {
    cat("\nSkipping", group, "because it has no points within the selected extent.\n")
    next
  }
  ...
  if (length(lineages) > 1) { # proceed with lineage models if there are multiple
    ...
    if (distance_method == "model-cost") {
      ...
      SDM.ras <- crop(SDM.ras, output_model_extent)
  ...
  if (! dir.exists(output_dir)) {
    ...
    cat("\n\nCreated new directory for lineage models:", output_dir, "\n\n")
  }
