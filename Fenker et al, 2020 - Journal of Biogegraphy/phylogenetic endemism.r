...
calc_sites_x_branches_from_models <- function(tree, sites_x_tips, core_count=1, include_root=T) {
  ...
  for (i in (nTips(tree)+1):branch.count) {     # this can be a foreach loop
    ...
    if (nodeType(tree)[i] == "root" & include_root == F) {next}
    ...
    new_col_num <- ncol(sites_x_branches)+1
    sites_x_branches[,new_col_num] <- branch_col
    names(sites_x_branches)[new_col_num] <- branch.labels[i]
    cat(i, "\t", branch.labels[i], "\t", length(desc), "\n")
    gc(verbose=F)
  }
...
# the tree has one data column called model.groups
  ...
  cat("\nPopulating sites x branches matrix\n")
  sites_x_branches <- calc_sites_x_branches_from_models(tree, sites_x_tips, core_count=1)
...
calc_sites_x_branches <- function(tree, sites_x_tips, core_count=1, include_root=T) {
  # tree must be a phylo4d tree containing only the tips for which there is a model of the same name
  # the tree has one data column called model.groups
